...
function MarketDiscovery() {
  ...
  }, [jobs, searchQuery, selectedCity, selectedMode])
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <select
                    ...
                    className="h-10 px-4 rounded-xl border border-slate-200 focus:border-primary outline-none text-xs font-bold text-slate-800 bg-white min-w-[150px]"
                  >
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            {filteredJobs.map((job) => {
              ...
              return (
                <Card key={job.id} className="relative overflow-hidden bg-white hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 rounded-3xl border border-slate-200/80 group p-6 select-none">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <Badge variant={job.location_type.toLowerCase() === 'remote' ? "default" : "secondary"} className="h-5 text-[8px]">
                            {job.location_type}
                          </Badge>
                        </div>

                        {/* Company & Location Details */}
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-slate-500 font-medium text-xs pt-0.5">
                          <div className="flex items-center gap-1.5">
                            ...
                            {job.company_website ? (
                              <a 
                                ...
                                className="hover:text-primary hover:underline transition-colors font-bold text-slate-700"
                              >
                                {job.company_name}
                              </a>
                            ) : (
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <span>{job.created_at ? new Date(job.created_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }) : "Recently"}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 self-start md:self-center shrink-0">
                      {matchScore !== null ? (
                        <div className="flex flex-col items-end gap-1">
                          <span className="text-[8px] font-black uppercase tracking-[0.2em] text-slate-400">Match score</span>
                          <Badge 
                            variant={matchScore >= 70 ? "shortlisted" : matchScore >= 45 ? "review" : "rejected"}
                            className="h-6 font-bold border"
                          >
                            {matchScore}% Match
                          </Badge>
                        </div>
                  ...
                  {job.core_skills && job.core_skills.length > 0 && (
                    <div className="mt-4 pt-3 border-t border-slate-100/60 flex flex-wrap gap-1.5">
                      {job.core_skills.map((skill, index) => {
                        ...
                        return (
                          <Badge 
                            ...
                            className={`text-[8px] px-2.5 py-1 uppercase font-bold transition-all ${
                              isMatched 
                                ? "bg-primary/10 border-primary/20 text-primary shadow-none" 
